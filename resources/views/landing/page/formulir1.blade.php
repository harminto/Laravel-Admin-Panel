...
<section class="formulir" id="formulir">
    ...
    <div class="box-container">
        <div class="box">
            <form action="{{ route('pendaftaran-festival') }}" method="POST" enctype="multipart/form-data" id="formPendaftaran">
                @csrf
                <input type="hidden" name="competition_types_id" value="{{ $competitionTypes->id }}">
                <div class="inputBox">
                    <input type="text" name="nama_kelompok" id="nama_kelompok" placeholder="Nama Kelompok Musik" required>
                    <input type="email" name="email_kelompok" id="email_kelompok" placeholder="email Kelompok" required>
                </div>
                <div class="inputBox">
                    <select name="lagu_wajib_id" id="lagu_wajib_id" required>
                        <option value="">--Pilih Lagu Wajib--</option>
                        @foreach ($laguWajibs as $laguWajib)
                            <option value="{{ $laguWajib->id }}">{{ $laguWajib->judul_lagu }} -> {{ $laguWajib->artis_pencipta }}</option>
                        @endforeach
                    </select>
                    <input type="text" name="lagu_pilihan" id="lagu_pilihan" placeholder="Masukkan Lagu Pilihan" required>
                </div>
                <div class="inputBox">
